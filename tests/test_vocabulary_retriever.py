...
class TestVocabularyNameRetriever:
    ...
    def test_retrieve_data_from_internet(self):
        word = "滋賀県"
        data_retriever = VocabularyDataRetriever(3, "jpn", "eng")
        data = data_retriever.start(word)
        assert data == ["滋賀県の県庁所在地は大津市です。", 
                        "Shiga Prefecture's capital is Otsu City.", 
                        "[滋賀|し|が][県|けん]の[県庁|けん|ちょう][所在地|しょ|ざい|ち]は[大津|おお|つ][市|し]です。", 
                        "Shiga Prefecture (Kinki area)",
                        "Noun"]
    
    def test_retrieve_data_from_internet_one(self):
        word = "滋賀県"
        data_retriever = VocabularyDataRetriever(3, "jpn", "eng")
        data = data_retriever.start(word)
        assert data[0][0] == "滋賀県の県庁所在地は大津市です。"
        
    def test_retrieve_data_from_internet_two(self):
        word = "滋賀県"
        data_retriever = VocabularyDataRetriever(3, "jpn", "eng")
        data = data_retriever.start(word)
        assert data[1][0] == "Shiga Prefecture's capital is Otsu City."
        
    def test_retrieve_data_from_internet_three(self):
        word = "滋賀県"
        data_retriever = VocabularyDataRetriever(3, "jpn", "eng")
        data = data_retriever.start(word)
