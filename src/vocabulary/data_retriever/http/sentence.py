...
def create_sentences_html_request(word, lang_from, lang_to, page = 1):
    ...
    http_request = "https://tatoeba.org/en/api_v0/search?from=" + lang_from + "&to=" + lang_to + "&query=" + word + "&page=" + str(page)
    ...
    for i in range(sentences_count): 
        ...
        if check_word_contains_kana(word) or word in this_lang_from_sentence: # Add sentence if word does appear in sentence
            ...
            while(json_sentences['results'][i]["translations"][j][k] == []): # Same
                k += 1
            one_sentence_data.append(json_sentences['results'][i]["translations"][j][k]["text"]) # Translation
            
            one_sentence_data.append(json_sentences['results'][i]["transcriptions"][0]["text"]) # Transcription
...
def get_paging_data(json_data):
    ...
    current_page_items_number = int(sentences_data["current"]) # Number of items displayed in the current page
...
def get_sentences_http(word, lang_from, lang_to):
    ...
    (current_page, current_page_items_number, count, perPage) = get_paging_data(json_data)
    ...
    for i in range(2, total_page_number):
        t = threading.Thread(target=retrieve_and_get_sentences_data, args=((word, lang_from, lang_to, i, data),))
        t.start()
        threads.append(t)
