...
def get_sentence_anki_format(sentence_str, kanji_data):
    ...
    for word_data in kanji_data:
        ...
        if word_position != -1:
            ...
            sentence_anki_format = sentence_anki_format[:position] + \
                word_anki_format + " " + sentence_anki_format[position:]
            added_word_index += len(word) + len(word_anki_format) + 1
            if word_position != 0 and word_position != " ":
                sentence_anki_format = sentence_anki_format[:word_position] + \
                    " " + sentence_anki_format[word_position:]
